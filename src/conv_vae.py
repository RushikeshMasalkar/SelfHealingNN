...
class ConvEncoder(nn.Module):
    def __init__(self, latent_dim: int = 256):
        super().__init__()
        # Improved 3-stage downsampling: 32x32 -> 16x16 -> 8x8 (Final spatial dim)
        self.features = nn.Sequential(
            ...
            nn.BatchNorm2d(256),
            nn.LeakyReLU(0.2, inplace=True),
            # Block 3: 8x8 -> 8x8 (Preserve spatial resolution)
            nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True),
        )
        self.flatten = nn.Flatten()
        # 512 channels * 8 * 8 spatial = 32768
...
class ConvDecoder(nn.Module):
    def __init__(self, latent_dim: int = 256):
        ...
        # Project back to 512 * 8 * 8 = 32768
        self.fc2 = nn.Linear(1024, 512 * 8 * 8)
        # Upsampling blocks: 8->16->32 spatial dims
        self.deconv = nn.Sequential(
            ...
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            # Block 3: 32x32 -> 32x32 (Detail refinement)
...
def vae_loss(
    ...
    # Prioritizing L1 (90%) for sharper reconstructions
    recon_loss_per_sample = 0.9 * recon_l1 + 0.1 * recon_mse
